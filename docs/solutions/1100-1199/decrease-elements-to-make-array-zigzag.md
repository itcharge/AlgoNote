...
它必须比左右两边相邻的元素都小。所以它最终的值最多是 $\min(\text{左边邻居}, \text{右边邻居}) - 1$。如果它当前的值已经小于这个值了，就不用减了；否则需要减少：当前值 - 目标值。
...
2. 遍历数组的每个位置：
   - 找到左右邻居的值（越界的视为无限大）。
   - 计算 $\min(\text{左邻居}, \text{右邻居})$。
   - 如果当前值 >= 这个最小值，就需要减少到 $\min(\text{左邻居}, \text{右邻居}) - 1$，操作次数 = 当前值 - 目标值。
   - 把这个操作次数累加到对应形态的结果上。
3. 最后取两个结果中的较小值。
