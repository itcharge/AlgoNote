...
- **查找操作**：直接比较两个元素的 $id$ 是否相同，若相同则属于同一集合，否则属于不同集合。
...
- **查找操作**：从某个元素出发，沿着 $fa$ 数组不断查找其父节点，直到找到根节点。若两个元素的根节点相同，则它们属于同一集合，否则属于不同集合。
...
虽然路径压缩能够有效降低树的高度，但它只在查找操作时生效，且仅影响当前查找路径上的节点。因此，若仅依赖路径压缩，整个并查集的结构仍可能出现较高的树。为进一步优化并查集的结构，常用的另一种方法是「按秩合并」。
...
class UnionFind:
    ...
    def union(self, x, y):
        ...
        :param x: 元素 x
        :param y: 元素 y
        :return: 如果合并成功返回 True，若已在同一集合返回 False
        """
        root_x = self.find(x)
        root_y = self.find(y)
...
- **空间复杂度**：主要由数组 $fa$（父节点数组）构成，若采用「按秩合并」优化，还需额外的 $rank$ 或 $size$ 数组。整体空间复杂度为 $O(n)$，其中 $n$ 为元素个数。
