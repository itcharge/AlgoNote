...
> **背包问题求字典序最小的具体方案**：在背包问题中，除了要求最大价值外，有时还要求输出所有满足条件的方案中，按照物品编号排列后字典序最小的那一个。这里的「字典序最小」是指：将选中物品的编号（编号范围为 $0 \sim size - 1$）从小到大排列，得到的序列在所有可行方案中字典序最小。

例如，如果有两种可行方案分别为 [0,2,3] 和 [0,1,4]，则 [0,1,4] 的字典序更小。求解此类问题时，通常需要在动态规划的基础上，设计合理的回溯或记录路径方式，确保最终输出的方案满足字典序最小的要求。

#### 思路 5：动态规划 + 路径记录（输出字典序最小的方案）
